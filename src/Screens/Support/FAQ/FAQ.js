...
//   ];
  ...
  const fetchFaq = async(categoryId, categoryTitle) => {
    ...
    await axios.get(`https://news-manage.dyp.finance/api/faqs/${categoryId}`).then((res)  => {
        ...
        <div className="categories-container px-0 p-5 w-100">
          ...
          <div className="row categories-grid mt-5 gap-4">
           {categories.map((category, index) => (
             <div onClick={() => fetchFaq(category.id, category.category)} key={index} className={`category-card d-flex justify-content-center ${faqTitle === category.category ? 'active-category' : null } align-items-center flex-column gap-2 mx-auto`}>
             <img src={faqTitle === category.category ? activeBook : book} alt="" />
